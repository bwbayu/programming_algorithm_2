...
int read(char string[101], film data[51])
{
	int n = 0;
	FILE *fdata;
...
int read1(char string[101], film data[51])
{
	int n = 0;
	FILE *fdata;
	fdata = fopen(string, "r");
...
void write(char str[51], int n, film data[51])
{
	FILE *fdata;
	fdata = fopen(str, "w");
	for (int i = 0; i < n; i++)
...
void write1(char str[51], int n, film data[51])
{
	FILE *fdata;
	fdata = fopen(str, "w");
	for (int i = 0; i < n; i++)
...
int maxstr(int n, char arr[51][101])
{
...
void garis(int totalspasi)
{
	for (int i = 0; i < totalspasi; i++)
		printf("-");
}

void spasi(int spasitabel, char str[101])
{
...
void print(int max[4], int total, film data[51], int bdata)
{
	garis(total);
	// nama tabel
	printf("\n|Judul");
...
void sort(int kiri, int kanan, film data[51])
{
	int i, j;
	film temp;
	i = kiri, j = kanan;
	while (data[i].id < data[kanan].id && i <= j)
	{
		i++;
	}
	while (data[j].id > data[kiri].id && i <= j)
	{
		j--;
	}
	if (i < j)
...
void cari(char id[10], char str[51])
{
...
char cek(char temp[100], int n, film data[51])
{
	int i = 0, j = 0;
...
void appends(int n, film data[51])
{
	char sign, temp[100];
	printf("Masukkan id sutradara: ");
	scanf("%s", temp);
	sign = cek(temp, n, data);
	if (sign == 'y')
	{
		n++;
...
void appendg(int n, film data[51])
{
	char sign, temp[100];
...
void appendb(int n, film data[51])
{
	char sign, temp[100];
