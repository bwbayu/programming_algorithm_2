...
void updatechar(game data, game character[], int x) // cari primary key, ganti data lain
{
    strcpy(character[x].job_char, edata.job_char);
    strcpy(character[x].skill, edata.skill);
}

void deluser(game edata, game user[], int x) // cari primary key, dibuat untuk hapus data di awal, akhir, tengah
{
...
void delchar(game data, game character[], int x) // cari primary key, dibuat untuk hapus data diawal, akhir, tengah
{
    for (int i = x; i < nchar - 1; i++)
    {
        strcpy(user[i].id_char, user[i + 1].id_char);
        strcpy(user[i].job_char, user[i + 1].job_char);
        strcpy(user[i].skill, user[i + 1].skill);
    }
    strcpy(user[nchar - 1].id_char, "\0");
    strcpy(user[nchar - 1].job_char, "\0");
    strcpy(user[nchar - 1].skill, "\0");
    nchar -= 1;
}
        ...
        else // untuk query entry, ubah, hapus
        {
            if (strcmp(getkata(), "user") == 0)
            {
                ...
                if (eop(str) == 0) // cek eop
                {
                    ...
                    else if (ins1 == 0 && ins2 == 0 && ins3 == 1 && ins4 == 0) // entry hapus
                    {
                        if (find == 1) // jika data ditemukan
                        {
                    ...
                    {
                        printf("Silahkan memperbaiki query, perintah mana yang ingin dijalankan program?");
                    }
                }
                else
                {
